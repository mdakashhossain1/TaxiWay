            ...
            <div class="hidden xl:block relative"
                x-data="{
                    ...
                    get groups() {
                        ...
                        return Object.entries(byGroup).map(([name, items]) => ({ name, items }));
                    },
                    search() {
            ...
            >
                <div class="relative">
                    ...
                    <input
                        ...
                        autocomplete="off"
                        placeholder="Search drivers, bookings, customers..."
                ...
                <div x-show="open" x-cloak x-transition
                    class="absolute left-0 right-0 z-50 mt-2 max-h-[420px] overflow-y-auto rounded-lg border border-gray-200 bg-white py-2 shadow-theme-lg dark:border-gray-800 dark:bg-gray-900 xl:w-[430px]">
                    <p x-show="loading" class="px-4 py-2 text-theme-sm text-gray-400">Searching…</p>
                    <p x-show="!loading && results.length === 0" class="px-4 py-2 text-theme-sm text-gray-400">
                        No results for "<span x-text="query"></span>"
                    </p>
                    <template x-for="group in groups" :key="group.name">
                        <div>
                            <p class="px-4 pt-2 pb-1 text-theme-xs font-medium uppercase text-gray-400" x-text="group.name"></p>
                            <template x-for="item in group.items" :key="item.url">
                                <a :href="item.url" class="flex flex-col px-4 py-2 hover:bg-gray-50 dark:hover:bg-white/5">
                                    <span class="text-theme-sm font-medium text-gray-800 dark:text-white/90" x-text="item.title"></span>
                                    <span x-show="item.subtitle" class="text-theme-xs text-gray-500 dark:text-gray-400" x-text="item.subtitle"></span>
                                </a>
                            </template>
                        </div>
                    </template>
